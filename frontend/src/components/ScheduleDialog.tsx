...
export function ScheduleDialog({ schedule, initialStart, onClose, onSave, onDelete, onToggleComplete }: ScheduleDialogProps) {
  const [form, setForm] = useState(() => createForm(schedule, initialStart))
  const [error, setError] = useState('')
  const [saving, setSaving] = useState(false)
  const [confirmDelete, setConfirmDelete] = useState(false)

  useEffect(() => setForm(createForm(schedule, initialStart)), [schedule, initialStart])
      ...
      <form className="schedule-dialog" onSubmit={submit} role="dialog" aria-modal="true" aria-labelledby="schedule-dialog-title">
        ...
        <div className="dialog-body">
          <div className="form-field form-field--title">
            ...
            <input id="schedule-title" autoFocus maxLength={120} value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} placeholder="准备做什么？" />
          </div>

          <div className="form-grid">
            <DateTimePicker label="开始时间" value={form.startAt} onChange={(startAt) => setForm({ ...form, startAt })} />
            <DateTimePicker label="结束时间" value={form.endAt} onChange={(endAt) => setForm({ ...form, endAt })} />
          </div>

          <div className="form-grid">
            <div className="form-field">
              <label htmlFor="schedule-location">地点</label>
              <div className="input-with-icon"><MapPin size={16} /><input id="schedule-location" maxLength={200} value={form.location} onChange={(event) => setForm({ ...form, location: event.target.value })} placeholder="添加地点" /></div>
            </div>
            <ReminderPicker value={form.reminderOffsets} onChange={(reminderOffsets) => setForm({ ...form, reminderOffsets })} />
          </div>

          <div className="form-field">
            <label htmlFor="schedule-notes">备注</label>
            <textarea id="schedule-notes" maxLength={4000} rows={3} value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} placeholder="补充日程信息" />
          </div>

          <fieldset className="colour-field">
            <legend>颜色</legend>
            <div>{COLOURS.map((colour) => <button type="button" key={colour} className={form.color === colour ? 'is-selected' : ''} style={{ background: colour }} onClick={() => setForm({ ...form, color: colour })} aria-label={`选择颜色 ${colour}`}>{form.color === colour && <Check size={13} />}</button>)}</div>
          </fieldset>
        ...
        <footer className="dialog-footer">
          <div className="dialog-footer__secondary">
            ...
            {schedule && <button type="button" className="button button--ghost" onClick={() => onToggleComplete(schedule)} disabled={saving}><Check size={15} />{schedule.status === 'completed' ? '恢复未完成' : '标记完成'}</button>}
          </div>
